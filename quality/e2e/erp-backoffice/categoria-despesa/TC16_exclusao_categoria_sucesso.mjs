...
const OUTPUT_FILE = `${OUTPUT_DIR}/ui-${new Date().toISOString().replace(/[-:T\.]/g, "").slice(0, 15)}-categoria-despesa-exclusao-sucesso.png`;
...
async function main() {
  ...
  try {
    ...
    console.log("6. Looking for the last category in the table to delete...");
    ...
    if (rowCount > 0) {
      // Select the last row (assuming it's safe to delete)
      const lastRow = rows.nth(rowCount - 1);
      
      // Click the delete button in the row
      const deleteButton = lastRow.locator('button:has-text("Excluir"), button[aria-label*="excluir" i]');
