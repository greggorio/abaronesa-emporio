...
import ptBR from './delivery.pt-BR.json';
import enUS from './delivery.en-US.json';
import esES from './delivery.es-ES.json';
...
const localStorageKey = 'delivery:lang';
const canUseWindow = typeof window !== 'undefined';
const LOCALE_CHANGE_EVENT = 'delivery:locale-change';
...
export function useDeliveryI18n() {
  ...
  useEffect(() => {
    ...
    const handler = (event: Event) => {
      const detail = (event as CustomEvent<LocaleTag>).detail;
      const normalized = normalizeLocaleTag(detail);
      if (normalized) {
