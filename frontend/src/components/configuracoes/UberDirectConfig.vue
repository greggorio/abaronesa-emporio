<template>
  <div class="uber-config">
    <q-card class="config-card q-mb-lg" flat bordered>
      <q-card-section class="card-header">
        <div class="header-content">
          <div>
            <div class="text-h5 text-weight-medium text-grey-9">Uber Direct</div>
            <div class="text-body2 text-grey-7 q-mt-xs">Configurações da integração Uber Direct</div>
          </div>
          <q-icon name="delivery_dining" size="48px" color="grey-6" class="header-icon" />
          ...
          <template v-slot:body-cell-nome="props">
            <q-td :props="props" class="config-name-cell">
              <div class="config-name">
                <q-icon name="settings" size="18px" color="grey-7" class="q-mr-sm" />
              ...
              <template v-else>
                <div class="config-value" @click="startEditing(props.row)">
                  <span class="value-text">
                    {{ truncatedValue(props.value || "Não definido") }}
          ...
          <template v-slot:no-data>
            <div class="full-width row flex-center text-grey-6 q-gutter-sm q-pa-lg">
              ...
              <span class="text-body1">
                {{ searchFilter ? "Nenhuma configuração encontrada" : "Nenhuma configuração Uber disponível" }}
              </span>
            </div>
          </template>
        </q-table>
...
const isBooleanValue = (val) => val === "true" || val === "false";
...
const startEditing = (row) => {
  if (isBooleanValue(row.valor)) return;
...
const loadUberConfigs = async () => {
  try {
    ...
    if (response) {
      uberConfigs.value = response.filter((config) =>
        config.chave.startsWith("uber_")
      );
    }
  } catch (error) {
    console.error("Erro ao carregar configurações Uber:", error);
    $q.notify({
      type: "negative",
      message: "Erro ao carregar configurações Uber",
      position: "top-right",
    });
  } finally {
    loading.value = false;
