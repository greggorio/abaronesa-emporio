<template>
  <div class="mercadopago-configurations">
    <q-card class="config-card q-mb-lg" flat bordered>
      <q-card-section class="card-header">
        <div class="header-content">
          <div>
            <div class="text-h5 text-weight-medium text-primary">Configurações do Mercado Pago</div>
            <div class="text-body2 text-grey-7 q-mt-xs">Configure os parâmetros da integração com o Mercado Pago</div>
          </div>
      ...
      <q-card-section class="q-pa-md q-gutter-sm actions-row">
        <div class="row items-center justify-between">
          <div>
            <div class="text-h6 text-weight-medium">Testes rápidos (Mercado Pago)</div>
            <div class="text-body2 text-grey-7">Executa tokenização e um pagamento de R$1,00 em sandbox (cartão de teste).</div>
          </div>
...
const filteredConfigs = computed(() => {
  if (!searchFilter.value) {
    return mercadoPagoConfigs.value;
  }
  const filter = searchFilter.value.toLowerCase();
  return mercadoPagoConfigs.value.filter(
    (config) =>
...
const updateConfig = async (config) => {
  ...
  await loadMercadoPagoConfigs();
};

const loadMercadoPagoConfigs = async () => {
  loading.value = true;
  try {
    const response = await apiRequest("/api/configs");
    if (response) {
      mercadoPagoConfigs.value = response.filter((config) =>
        config.chave.startsWith("mercadopago_")
      );
    }
  } catch (error) {
    ...
    $q.notify({
      type: "negative",
      message: "Erro ao carregar configurações do Mercado Pago",
      position: "top-right",
    });
  } finally {
    loading.value = false;
...
const runAllTests = async () => {
  ...
  try {
    const tokenResp = await apiRequest("/api/v1/mercadopago/create-token", "POST", {
    ...
    if (tokenValue) {
      testResults.value.token = { status: "success", message: "Token gerado com sucesso." };
    } else {
  ...
  try {
    const payResp = await apiRequest("/api/v1/mercadopago/payments", "POST", {
      transaction_amount: 1.0,
      payment_method_id: "visa",
      installments: 1,
      token: tokenValue,
      description: "Teste Mercado Pago R$1,00",
      external_reference: externalReference,
      payer: {
        email: "buyer-teste@example.com",
        first_name: "Teste",
        last_name: "MercadoPago",
        identification: { type: "CPF", number: "12345678909" },
      },
    });
    const status = payResp?.payment?.status || payResp?.status || "desconhecido";
    const providerId = payResp?.payment?.id || "-";
    testResults.value.status = {
      status: "pending",
      message: `Ref: ${externalReference} | providerId: ${providerId}`,
    };
    if (status === "approved" || status === "in_process" || status === "pending") {
      testResults.value.payment = {
        status: "success",
        message: `Status: ${status} | providerId: ${providerId}`,
      };
      await checkStatusBackend(externalReference);
    } else {
      testResults.value.payment = {
        status: "error",
        message: `Status: ${status} | providerId: ${providerId} | msg: ${payResp?.message || ""}`,
      };
    }
  } catch (error) {
    testResults.value.payment = {
...
const checkStatusBackend = async (externalReference) => {
  try {
