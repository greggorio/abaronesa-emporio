<template>
  <div class="nfce-configurations">
    <!-- Seção de Configurações -->
    <q-card class="config-card q-mb-lg" flat bordered>
      <q-card-section class="card-header">
        <div class="header-content">
          <div>
            <div class="text-h5 text-weight-medium text-primary">Configurações de Nota Fiscal de Consumidor Eletrônica</div>
            <div class="text-body2 text-grey-7 q-mt-xs">Configure os parâmetros para emissão de NFC-e</div>
          </div>
...
const updateConfig = async (config) => {
  ...
  await loadNfceConfigs();
};

const loadNfceConfigs = async () => {
  try {
    loading.value = true;
    ...
    if (response) {
      nfceConfigs.value = response.filter((config) => config.chave.startsWith("nfce_"));
    }
  } catch (error) {
    console.error("Erro ao carregar configurações de NFCe:", error);
    $q.notify({
      type: "negative",
      message: "Erro ao carregar configurações de NFCe",
      position: "top-right",
    });
  } finally {
    loading.value = false;
...
const testarConexao = async () => {
  try {
    ...
    const apiURL = `/api/nfce/teste-conexao`;
    const response = await apiRequest(apiURL);
...
const validarCertificado = async () => {
  try {
    ...
    const apiURL = `/api/nfce/valida-certificado`;
    const response = await apiRequest(apiURL);
