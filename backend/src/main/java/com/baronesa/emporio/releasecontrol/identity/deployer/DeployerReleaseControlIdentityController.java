...
public class DeployerReleaseControlIdentityController {

    private final DeployerReleaseControlIdentityService service;

    public DeployerReleaseControlIdentityController(DeployerReleaseControlIdentityService service) {
        this.service = service;
    }

    @GetMapping("/jwks")
    public ResponseEntity<JwksResponse> jwks() {
        DeployerReleaseControlIdentityKeyMaterial material = service.keyMaterial();
        JwkKey key = new JwkKey(
                "RSA",
                "sig",
                "RS256",
                material.keyId(),
                DeployerReleaseControlIdentityConfiguration.base64UrlUnsigned(
    ...
    public TokenResponse token(
        ...
        if (!(authentication.getPrincipal() instanceof UserPrincipal principal)
            ...
            throw new AccessDeniedException("deployer identity requires SYSTEM");
        }
        return new TokenResponse(
                service.issue(principal.getId()),
                "Bearer",
                DeployerReleaseControlIdentityService.TTL_SECONDS,
                DeployerReleaseControlIdentityService.SCOPE
        );
    }
